    ...
    <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 px-5">
            <div class="md:flex md:items-center md:justify-between mb-8">
                <div class="flex-1 min-w-0">
                    <h1 class="text-3xl font-bold text-primary-700">{{ $t('Create New FAQ') }}</h1>
                    <p class="mt-2 text-sm text-secondary-600">{{ $t('Add a new question and answer to the FAQ system.') }}</p>
                </div>
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-xl rounded-lg overflow-hidden">
                ...
                <form @submit.prevent="saveFaq" class="p-8 space-y-8" v-if="!loadingInitialData">
                    <!-- Question Field -->
                    <div class="space-y-2">
...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Create New FAQ')
        }
    },
    data() {
        return {
            loadingInitialData: false, // No initial data to load for new FAQ
            saving: false,
            form: {
                question: '',
                answer: '',
    ...
    methods: {
        ...
        async saveFaq() {
            ...
            try {
                const response = await axios.post('/api/dashboard/admin/faqs', payload);
                this.successMessage = response.data.message || this.$i18n.t('FAQ created successfully!');
                this.$notify({
