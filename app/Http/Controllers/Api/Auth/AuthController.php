...
class AuthController extends Controller
{
    ...
    public function register(RegisterRequest $request): JsonResponse
    {
        ...
        $user->condo_location_id = $request->get('condo_location_id');
        $user->password = bcrypt($request->get('password'));
        $user->role_id = Setting::getDecoded('app_default_role');

        // For new registrations, we'll require email verification
        // Don't set email_verified_at here - it will be set when they verify
        $user->save();

        // Send email verification notification
        $objNotificationData = new stdClass();
        $objNotificationData->user = $user;
        $user->notify((new VerifyEmail($objNotificationData))->locale(Setting::getDecoded('app_locale')));
